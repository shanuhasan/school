...
class AssignClassTeacher extends Model
{
    ...
    {
        return self::select('assign_class_teachers.*', 'mst_classes.name as class_name', 'mst_subjects.name as subject_name', 'mst_subjects.type as subject_type', 'mst_classes.id as class_id', 'mst_subjects.id as subject_id')
            ->join('mst_classes', 'mst_classes.id', 'assign_class_teachers.class_id')
    ...
    static public function getTeacherClassTimetable($classId, $subjectId)
    {
        ...
        $model = ClassSubjectTimetable::getClassAndSubject($classId, $subjectId, $week->id);
        return $model;
    }
}
