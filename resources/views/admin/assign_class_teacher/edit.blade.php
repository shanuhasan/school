@extends('layouts.app')
@section('title', 'Edit Assign Class To Teacher')
...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid my-2">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Edit Assign Class To Teacher</h1>
                </div>
                                    ...
                                    <select name="class_id" id="class_id" class="form-control">
                                        ...
                                        @foreach ($getClass as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $teacher->class_id == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="error"></p>
                                    ...
                                    <div>
                                        @foreach ($getTeacher as $item)
                                            @php
                                                $checked = '';
                                            @endphp
                                            @foreach ($teacherIds as $assignTeacher)
                                                @if ($assignTeacher->teacher_id == $item->id)
                                                    @php
                                                        $checked = 'checked';
                                                    @endphp
                                                @endif
                                            ...
                                            <input {{ $checked }} type="checkbox" value="{{ $item->id }}"
                                                id="teacher_id" name="teacher_id[]"> {{ $item->name }}
                                            {{ $item->last_name }} <br>
                                        @endforeach
                                    </div>
...
@section('script')
    <script>
        $('#assignClassTeacherForm').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('admin.assign_class_teacher.update', $teacher->id) }}",
                type: 'post',
